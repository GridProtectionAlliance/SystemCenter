...
const ByEmailType = (props: IProps) => {
    ...
    }, [emailTypeController.Fetch, refreshTrigger]);
    ...
    }, [search, sortField, asc, page, emailTypeController.PagedSearch, refreshTrigger])

    React.useEffect(() => {
        let e = [];
        if (newEmail.Name == undefined || newEmail.Name.length < 1)
            e.push('A Name is required.');
        if (newEmail.Name != null && newEmail.Name.length > 50)
        ...
        if (newEmail.FilePath != null && newEmail.FilePath.length > 200)
