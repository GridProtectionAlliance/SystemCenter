...
const triggeredEmailDataSourceController = new GenericController<IDataSourceTriggeredEmailType>(`${homePath}api/OpenXDA/TriggeredEmailDataSourceEmailType`, "TriggeredEmailDataSourceName", false);
const triggeredDataSourceSettingController = new GenericController<ITriggeredEmailDataSourceSetting>(`${homePath}api/OpenXDA/TriggeredEmailDataSourceSetting`, "Name", false);
    ...
    function StandardUI() {
        return <div className="row">
                ...
                {record != null ? <Select<IDataSourceTriggeredEmailType> Record={record} Field={'TriggeredEmailDataSourceID'} Options={types.map(t => ({ Value: t.ID.toString(), Label: t.Name }))}
                    Setter={(r) => setRecord({ ...r, TriggeredEmailDataSourceID: parseInt(r.TriggeredEmailDataSourceID.toString()) })} Label={'Type'} Help={'The type of database to query. Contact GPA to inquire about additional types.'} /> : null}
                </div>
            </div>
    }
