...
export const FetchValueLists = createAsyncThunk('Companys/FetchValueLists', async (arg: { group: string }, { dispatch }) => {
    return await GetValueList(arg.group)
});
...
export const ValueListSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(FetchValueLists.fulfilled, (state, action) => {
            ...
            state[action.meta.arg.group].Data.push(action.payload);
        });
        builder.addCase(FetchValueLists.pending, (state, action) => {
            if (state[action.meta.arg.group] === undefined) {
                state[action.meta.arg.group] = {
                    Status: 'unitiated' as SystemCenter.Status,
                    Data: [] as SystemCenter.ValueListItem[],
...
export const SelectValueList = (state, group: string) => state.ValueList[group].Data as SystemCenter.ValueListItem[]
export const SelectValueListStatus = (state, group: string) => state.ValueList[group].Status as SystemCenter.Status
