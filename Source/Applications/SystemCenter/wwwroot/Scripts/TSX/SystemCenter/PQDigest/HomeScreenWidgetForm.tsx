...
import { AllWidgets } from '../../../../../EventWidgets/TSX/CollectionWidgetWrapper';
import { useAppDispatch, useAppSelector } from '../hooks';
import { PQDigestHomeScreenSlice } from '../Store/Store';
import { cloneDeep } from 'lodash';
import { EventWidget } from '../../../../../EventWidgets/TSX/global';
import { Alert } from '@gpa-gemstone/react-interactive';

interface IProps {
    Widget: LocalXDA.IHomeScreenWidget,
    stateSetter: (tab: LocalXDA.IHomeScreenWidget) => void,
    setErrors?: (e: string[]) => void,
}

export default function HomeScreenWidgetForm(props: IProps) {
    const dispatch = useAppDispatch();
    const allWidgets = useAppSelector(PQDigestHomeScreenSlice.Data);
    const allWidgetStatus = useAppSelector(PQDigestHomeScreenSlice.Status);
    ...
    return (
        <div className="col">
            <Input<LocalXDA.IHomeScreenWidget> Record={props.Widget} Setter={props.stateSetter} Field={'Name'}
                Valid={() => validName} />
            <Input<LocalXDA.IHomeScreenWidget> Record={props.Widget} Setter={props.stateSetter} Label={'Time Frame (days)'} Field={'TimeFrame'} Type={'integer'}
                Valid={() => props.Widget.TimeFrame > 0} />
            <Select<LocalXDA.IHomeScreenWidget> Record={props.Widget} Field={'Type'}
                Label='Type'
...
const WidgetSetting = (props: { Settings: {}, Type: string, SetSetting: (s: {}) => void, SetErrors: (e: string[]) => void }) => {
    const widget: EventWidget.ICollectionWidget<unknown> | undefined = React.useMemo(() => AllWidgets.find(w => w.Name == props.Type), [props.Type]);
