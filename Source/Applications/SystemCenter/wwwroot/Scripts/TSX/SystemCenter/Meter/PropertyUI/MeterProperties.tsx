...
const MeterProperties = (props: IProps) => {
    ...
    return (
            <div className="row">
                ...
                <div className="col">
                <Select<OpenXDA.Types.Meter> Record={props.Meter} Field={'Make'} Options={makeList.map(item => { return { Value: item.Value, Label: item.AltValue ?? item.Value, SortOrder: item.SortOrder ?? 0 } }).sort((a, b) => a.SortOrder - b.SortOrder)} Setter={(meter: OpenXDA.Types.Meter) => props.StateSetter(meter)} Disabled={hasPermissions()} />
                <Select<OpenXDA.Types.Meter> Record={props.Meter} Field={'Model'} Options={modelList.map(item => { return { Value: item.Value, Label: item.AltValue ?? item.Value, SortOrder: item.SortOrder ?? 0 } }).sort((a, b) => a.SortOrder - b.SortOrder)} Setter={(meter: OpenXDA.Types.Meter) => props.StateSetter(meter)} Disabled={hasPermissions()} />
